		...
		return (
			<Root className={classes.root}>
				<Paper className={classes.paper} elevation={0}>
					<Typography variant="h6" align="center" style={{fontFamily:'Nunito', fontWeight:'bold'}}>
						Welcome to Memory Diaries Please Sign In to create your memories with us!
					</Typography>
				</Paper>
			</Root>
		)
		...
		<Root className={dragging ? classes.drag : classes.root} onDragEnter={dragEnter} onDragOver={dragEnter} onDragLeave={dragLeave} onDrop={fileDrop}>
			<Paper className={classes.paper} elevation={0}>
				<form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
					<Typography style={{ color: 'black', fontWeight:'bold',padding:'10px',margin:'10px', fontSize:'25px', fontFamily:'Nunito' }} variant="h6">
					...
					<TextField sx={{ input: { color: 'black' } }} name="title" variant="outlined" label="Title" fullWidth value={postData.title} onChange={(e) => setPostData({ ...postData, title: e.target.value })} />
					<TextField InputProps={{ style: { color: 'black' } }} name="message" variant="outlined" label="Message" fullWidth multiline rows={4} value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value })} />
					<ChipInput fullWidth InputProps={{ style: { color: 'black' } }} value={postData.tags} newChipKeyCodes={[188, 13]} onAdd={handleAdd} onDelete={handleDelete} label="Tags" variant="outlined" className={classes.chip} />
